// 
// Original was: https://github.com/attestate/better-sqlite3-session-store
//
...
module.exports = ({ Store }) => {
  class SqliteStore extends Store {
    ...
    #startInterval() {
      ...
      setInterval(() => this.#clearExpiredSessions(), this.expired.intervalMs);
    }

    #clearExpiredSessions() {
      try {
        this.#client.prepare(`DELETE FROM ${tableName} WHERE datetime('now') > datetime(expire)`).run();
      } catch (err) {
        console.error(err);
      }
    }
    ...
    set(sid, sess, cb = noop) {
      ...
      try {
        res = this.#client
          .prepare(`INSERT OR REPLACE INTO ${tableName} VALUES (@sid, @sess, @expire)`)
          .run(entry);
      } catch (err) {
        cb(err);
        return;
    ...
    get(sid, cb = noop) {
      ...
      try {
        res = this.#client.prepare(`SELECT sess FROM ${tableName} WHERE sid = @sid AND datetime('now') < datetime(expire)`)
        .get({ sid });
      } catch (err) {
        cb(err);
        return;
    ...
    touch(sid, sess, cb = noop) {
      ...
      try {
        res = this.#client
          .prepare(`UPDATE ${tableName} SET expire = @expire WHERE sid = @sid AND datetime('now') < datetime(expire)`)
          .run(entry);
      } catch (err) {
        cb(err);
        return;
